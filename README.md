...
⚠️ **Important information:**  
To avoid situations where error registration in the `@errorHandler`meta expression handler may be performed incorrectly, the local variable `ParseError? error` is  intended for this purpose. Also, to implement the ability to rollback last errors, the local variable `rollbackErrors` will be available.
...
HexNumber = @errorHandler(HexNumberRaw, {
    rollbackErrors = true;
    error = ErrorMessage(start - state.failPos, 'Expected 4 digit hex number');
  }) ;
```
...
The meta expression `@expected` is a shorthand version of an error handler that generates an error `ErrorExpectedTags([tag])` and remove all errors generated by the processed expression if parsing of the expression being processed fails and the expression being processed does not consume any input data. In all other cases meta expression `@expected` does nothing.  
...
The above example, if there is a parse error, will generate the following error.
