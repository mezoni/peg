...
- Добавлены примеры реализации `Utf8Reader` и `FileReader`.

## 1.0.11

- Удалено выражение `AndPredicateActionExpression` по причине того, что с помощью мета-выражения `@verify` легко реализуется функционал данного выражения.

## 1.0.10

- Коррекция в проверке возможности оптимизации кода, генерируемого генератором `OrderedChoiceGenerator`.
- Сгенерирован новый парсер PEG, сгенерированы файлы примеров.
- Добавлено мета-выражение `@verify`.

## 1.0.9

- Оптимизация кода, генерируемого генератором `OrderedChoiceGenerator`.

## 1.0.8

- Изменена реализация обработчика ошибок. Обработчик ошибок реализован через мета-выражение. Также новый обработчик сделан более надежным и простым для понимания.
- В синтаксис PEG грамматики добавлена концепция мета-выражений.

## 1.0.7

- Добавлено выражение `AndPredicateActionExpression`.

## 1.0.6

- Отменена оптимизация кода, генерируемого генератором `SliceGenerator`.
- Изменения в файле примера грамматики `json_parser.peg`.
- Изменения в файле примера грамматики `calc_parser.peg`.

## 1.0.5

- Оптимизация кода, генерируемого генератором `OptionalGenerator`.
- Оптимизация кода, генерируемого генератором `ZeroOrMoreGenerator`.
- Изменения в файле примера грамматики `json_parser.peg`.


## 1.0.4

- Добавлена документация для универсальных функций разбора верхнего уровня из `runtime` кода.

## 1.0.3

- Исправлена ошибка в примере грамматики `csv_parser.peg`.

## 1.0.2

- Изменения в грамматике PEG, касающиеся экранированных символов.

## 1.0.1

- Добавлен пример CSV парсера.
