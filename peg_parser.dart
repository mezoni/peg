...
class PegParser {
  ...
  Expression? parseSequence(State<StringReader> state) {
    Expression? $0;
    // e:SequenceElement+ a:Action? h:ErrorAction?
    final $1 = state.pos;
    List<Expression>? $2;
    final $5 = <Expression>[];
    while (true) {
    ...
    if (state.ok) {
      ...
      if (state.ok) {
        ...
        if (state.ok) {
          ...
          final a = $3;
          final h = $4;
          $$ = SequenceExpression(expressions: e, action: a, handler: h);
          $0 = $$;
        }
      }
    }
...
class State<T> {
  ...
  void removeLastErrors(int failPos, int errorCount) {
